...
class OSGPR_VFE(gpflow.models.GPModel, gpflow.models.InternalDataTrainingLossMixin):
    """
    ...
    def __init__(self, data, kernel, mu_old, Su_old, Kaa_old, Z_old, Z, mean_function=None):
        ...
        num_latent_gps = gpflow.models.GPModel.calc_num_latent_gps_from_data(data, kernel, likelihood)
    ...
    def _common_terms(self):
        ...
        # s is test points
        Kbf = gpflow.covariances.Kuf(self.inducing_variable, self.kernel, self.X)
        Kbb = gpflow.covariances.Kuu(self.inducing_variable, self.kernel, jitter=jitter)
        Kba = gpflow.covariances.Kuf(self.inducing_variable, self.kernel, self.Z_old)
...
def plot_VFE_optimized(M, use_old_Z, shuffle):
    ...
    model2 = OSGPR_VFE((X2, y2), gpflow.kernels.RBF(variance=model1.kernel.variance, lengthscales=model1.kernel.lengthscales), mu1, Su1, Kaa1, 
        Zopt, Zinit)
    ...
    model3 = OSGPR_VFE((X3, y3), gpflow.kernels.RBF(variance=model2.kernel.variance, lengthscales=model2.kernel.lengthscales), mu2, Su2, Kaa2, 
        Zopt, Zinit)
