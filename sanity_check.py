...
from gpflow_osgpr import GPflowModel
import gpflow
...
def gpflow_main():
    ...
    x_train, y_train, batch_size, x_test = get_data()
    fig, axes = plt.subplots(4, 1, sharex=True, figsize=(7, 7))
    axes[0].set_title("GPflow Prediction")

    online_model = GPflowModel(
        num_inducing=num_inducing,
        kernel=gpflow.kernels.RBF(variance=init_output_scale, lengthscales=init_length_scale),
        noise_variance=init_noise_variance,
    )
    ...
    for batch_index in range(3):
        ...
        mean, var = online_model.predict(x_test)
    ...
    # Train on the full dataset using SGPR
    print("Running GPflow SGPR on the full dataset for comparison...")
    batch_model = GPflowModel(
        num_inducing=num_inducing,
        kernel=gpflow.kernels.RBF(variance=init_output_scale, lengthscales=init_output_scale),
        noise_variance=init_noise_variance,
    )
    batch_model.fit(x_train, y_train)
    mean, var = batch_model.predict(x_test)
    plot_result(
        axes[3],
        x_train,
        y_train,
